...
## 更新摘要
**变更内容**
- 新增 AlarmCard 展开状态管理机制，支持滑动展开/折叠
- 实现自动折叠功能，提升用户体验
- 完善国际化字符串资源集成，支持中英文双语
- 更新 AlarmCard 组件的交互逻辑以支持展开状态同步
...
**更新** 本次更新重点反映了 AlarmCard 展开状态管理、自动折叠机制以及国际化字符串资源的完整集成。
...
subgraph "国际化资源"
Strings["res/values/strings.xml<br/>res/values-zh-rCN/strings.xml"]
end
Screens --> Components
Components --> Models
Components --> Utils
...
- [AlarmCard.kt:1-239](file://app/src/main/java/com/snuabar/sunrisesunsetalarm/ui/components/AlarmCard.kt#L1-L239)
...
## 核心组件
- HomeScreen：主界面容器，负责响应式状态管理、权限请求、协程调度、导航与底部弹窗/设置页的条件渲染，**新增** AlarmCard 展开状态跟踪与自动折叠机制。
- AddAlarmBottomSheet：底部弹窗，负责闹钟新增/编辑的表单构建、数据绑定、验证与保存回调。
- AlarmCard：单个闹钟项，负责展示、开关切换、点击编辑、删除确认等交互，**增强** 支持滑动展开/折叠状态管理。
...
- 屏幕容器（HomeScreen）持有全局状态与协程作用域，协调权限、通知与精确闹钟策略，**新增** 展开状态管理与自动折叠。
- UI组件通过参数注入回调函数，实现解耦的交互逻辑，**增强** AlarmCard 支持外部展开状态同步。
- 工具类（SunCalcUtil、LocationManagerHelper）提供计算与定位能力。
- 服务层（AlarmManagerHelper）负责闹钟调度与取消。
- **新增** 国际化字符串资源系统，支持中英文双语界面。

```mermaid
graph TB
Home["HomeScreen<br/>状态与协程<br/>权限与调度<br/>展开状态管理"] --> BottomSheet["AddAlarmBottomSheet<br/>表单与保存"]
Home --> Cards["AlarmCard<br/>开关/编辑/删除<br/>展开状态同步"]
...
- 响应式状态管理
  ...
  - **新增** 展开状态管理：`expandedAlarmId` 用于跟踪当前展开的 AlarmCard ID。
- 协程与异步
...
- **新增** 自动折叠机制
  - 当列表开始滚动时，自动将展开的 AlarmCard 折叠，提升用户体验。
...
HS-->>U : 列表更新
U->>Card : 滑动展开
Card->>HS : onExpandChanged(true)
U->>HS : 开始滚动列表
HS->>HS : expandedAlarmId = null (自动折叠)
```
...
- **国际化支持**
  - 全面使用 `stringResource(R.string.xxx)` 进行国际化字符串处理。
...
- **增强** 展开状态管理
  - 接收外部 `isExpanded` 状态，支持与 HomeScreen 同步。
  - 使用 Animatable 实现平滑的展开/折叠动画。
  - 支持手势滑动展开/折叠。
- 交互逻辑
...
- 组件耦合
  ...
  - **新增** AlarmCard 与 HomeScreen 之间建立展开状态同步机制。
- 外部依赖
...
- 响应式状态与重组
  ...
  - **新增** 展开状态管理使用 Animatable 实现高效动画，避免频繁重组。
- 异步与IO
...
- **新增** 展开状态不同步
  - 如果 AlarmCard 展开状态与预期不符，检查 HomeScreen 中 `expandedAlarmId` 状态管理逻辑。
- **新增** 国际化显示问题
  - 如果界面显示英文而非中文，检查设备语言设置或确认 strings.xml 文件完整性。
...
## 结论
该应用以HomeScreen为核心，结合Material Design 3与Jetpack Compose实现了现代化的UI交互体验。通过组合式组件与回调机制，实现了清晰的状态管理与职责分离；借助协程与权限系统，保障了异步操作与系统能力的正确使用。**本次更新增强了 AlarmCard 的交互体验，新增展开状态管理与自动折叠机制，显著提升了用户体验。同时，完善的国际化字符串资源系统确保了中英文双语支持的完整性。** AddAlarmBottomSheet、AlarmCard与LocationPicker三大组件覆盖了新增编辑、展示交互与位置选择的核心场景，具备良好的扩展性与可维护性。
...
- 实践建议
  ...
  - **新增** 展开状态管理：合理使用外部状态同步机制，避免状态冲突。
  - **新增** 国际化支持：确保所有用户可见文本都通过 stringResource 进行国际化处理。
  - **新增** 动画性能：使用 Animatable 和 remember 优化动画性能，避免不必要的重组。